"""
CASO BASE FIFO - VERSIÓN INTERMEDIA
Configuración: 10 zonas, 40 vehículos, 12 períodos (3 horas)
"""
...
print("="*80)
print("🚀 CASO BASE FIFO - VERSIÓN INTERMEDIA")
print("="*80)

# CONFIGURACIÓN INTERMEDIA
ZONAS_TEST = [87, 116, 137, 151, 128, 186, 162, 163, 164, 68]
NUMERO_VEHICULOS = 40
...
PERIODO_FIN = 44     # 11:00 AM
PERIODOS_SIMULACION = PERIODO_FIN - PERIODO_INICIO

print(f"\n📍 Configuración INTERMEDIA:")
print(f"   • Zonas: {len(ZONAS_TEST)} zonas - {ZONAS_TEST}")
print(f"   • Vehículos: {NUMERO_VEHICULOS}")
print(f"   • Períodos: {PERIODOS_SIMULACION} (8:00 AM - 11:00 AM)")
...
# Clases (copiadas de la versión expandida)
class Vehiculo:
...
class SimuladorRideHailing:
    ...
    def ejecutar_simulacion(self):
        ...
        for periodo_local in range(PERIODOS_SIMULACION):
            ...
            if periodo_local % 4 == 0 or periodo_local == PERIODOS_SIMULACION - 1:
                hora = (periodo_global * PERIODO_SIMULACION) // 60
    ...
    def calcular_kpis_finales(self):
        ...
        # Calcular distancia y tiempo promedio de viajes
        if self.kpis['viajes_atendidos'] > 0:
            self.kpis['distancia_promedio_viaje'] = self.kpis['km_totales'] / self.kpis['viajes_atendidos']
        else:
            self.kpis['distancia_promedio_viaje'] = 0
        
        # Estimar tiempo promedio (basado en distancia y velocidad promedio de ~30 km/h en Manhattan)
        if self.kpis['viajes_atendidos'] > 0:
            self.kpis['tiempo_promedio_viaje'] = (self.kpis['distancia_promedio_viaje'] / 30) * 60  # minutos
        else:
            self.kpis['tiempo_promedio_viaje'] = 0
    
    def imprimir_resultados(self):
        print("\n" + "="*80)
        print("📊 RESULTADOS CASO BASE (FIFO) - VERSIÓN INTERMEDIA")
        print("="*80)
        ...
        print("\n🛣️ KPI DE DISTANCIA Y TIEMPO:")
        print(f"   • Distancia promedio/viaje: {self.kpis['distancia_promedio_viaje']:.2f} km")
        print(f"   • Distancia total recorrida: {self.kpis['km_totales']:.2f} km")
        print(f"   • Tiempo promedio/viaje: {self.kpis['tiempo_promedio_viaje']:.2f} min")
        
        print("\n" + "="*80)
    
    def guardar_resultados(self):
        with open('/Users/benjaminreyes/UC/capstone-3/resultados_casobase_intermedio.txt', 'w') as f:
            f.write("RESULTADOS CASO BASE (FIFO) - VERSIÓN INTERMEDIA\n")
            f.write("="*80 + "\n\n")
            f.write(f"CONFIGURACIÓN:\n")
            f.write(f"  Zonas: {len(ZONAS_TEST)}\n")
            f.write(f"  Vehículos: {NUMERO_VEHICULOS}\n")
            f.write(f"  Períodos: {PERIODOS_SIMULACION} (3 horas)\n\n")
            f.write("KPI:\n")
            ...
            f.write(f"  Distancia promedio/viaje: {self.kpis['distancia_promedio_viaje']:.2f} km\n")
            f.write(f"  Distancia total: {self.kpis['km_totales']:.2f} km\n")
            f.write(f"  Tiempo promedio/viaje: {self.kpis['tiempo_promedio_viaje']:.2f} min\n")
        
        print(f"💾 Resultados guardados en: resultados_casobase_intermedio.txt")
