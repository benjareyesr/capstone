"""
SIMULACIÓN CASO BASE FIFO - VERSIÓN DE PRUEBA
Subconjunto reducido de zonas y períodos para comparación con modelo de optimización
"""
...
print("="*60)
print("🚀 SIMULACIÓN CASO BASE (FIFO) - VERSIÓN DE PRUEBA")
print("="*60)

# -------------------------
# PARÁMETROS DE PRUEBA (ALINEADOS CON MODELO)
# -------------------------

# ZONAS DE PRUEBA: Mismas 5 zonas del modelo
ZONAS_TEST = [87, 116, 137, 151, 128]
ZONA_TEST_A_INDICE = {zona_id: idx for idx, zona_id in enumerate(ZONAS_TEST)}

print(f"\n📍 Zonas de prueba: {ZONAS_TEST}")

NUMERO_VEHICULOS = 20  # Mismo número que el modelo
AUTONOMIA_VEHICULO = 350  # km
TIEMPO_RECARGA = 110  # minutos
PORCENTAJE_DEMANDA = 0.05
COSTO_REUBICACION_MULTIPLIER = 1.25
PERIODO_SIMULACION = 15  # minutos

# PERÍODOS: 8:00 AM a 10:00 AM (8 períodos - horario peak)
PERIODO_INICIO = 32  # 8:00 AM
PERIODO_FIN = 40     # 10:00 AM
PERIODOS_SIMULACION = PERIODO_FIN - PERIODO_INICIO

print(f"⏰ Períodos: {PERIODOS_SIMULACION} períodos (8:00 AM - 10:00 AM)")
...
# Estaciones de carga (mismas del modelo)
ZONAS_ESTACIONES_CARGA = [87, 116, 137, 151, 128]
...
class SimuladorRideHailing:
    ...
    def inicializar_flota(self) -> List[Vehiculo]:
        ...
        # Distribuir vehículos en zonas con estaciones
        vehiculos_por_zona = NUMERO_VEHICULOS // len(ZONAS_ESTACIONES_CARGA)
        vehiculos_restantes = NUMERO_VEHICULOS % len(ZONAS_ESTACIONES_CARGA)
        
        vehiculo_id = 0
        for idx, zona in enumerate(ZONAS_ESTACIONES_CARGA):
    ...
    def cargar_viajes_periodo(self, periodo_global: int, df_all: pd.DataFrame) -> List[Viaje]:
        ...
        # Usar datos reales: filtrar por hora del día (no fecha específica)
        # Esto agrega todos los viajes que ocurrieron a esa hora en cualquier día del dataset
        fecha_inicio_hora = hora
        fecha_inicio_min = min_inicio
        fecha_fin_hora = (periodo_global * PERIODO_SIMULACION + PERIODO_SIMULACION) // 60
        fecha_fin_min = (periodo_global * PERIODO_SIMULACION + PERIODO_SIMULACION) % 60
        
        # Filtrar por hora del día (agregando datos de todos los días)
        df_all['hora'] = df_all['pickup_datetime'].dt.hour
        df_all['minuto'] = df_all['pickup_datetime'].dt.minute
        
        viajes_periodo = df_all[
            ((df_all['hora'] == fecha_inicio_hora) & (df_all['minuto'] >= fecha_inicio_min)) |
            ((df_all['hora'] == fecha_fin_hora) & (df_all['minuto'] < fecha_fin_min))
        ]
        ...
        for _, row in viajes_muestra.iterrows():
            ...
            # Solo viajes dentro del subconjunto de prueba
    ...
    def imprimir_resultados(self):
        print("\n" + "="*60)
        print("📊 RESULTADOS SIMULACIÓN CASO BASE (FIFO)")
        print("="*60)
    ...
    def guardar_resultados(self):
        with open('/Users/benjaminreyes/UC/capstone-3/resultados_casobase_test.txt', 'w') as f:
            f.write("RESULTADOS CASO BASE (FIFO) - PRUEBA\n")
            f.write("="*60 + "\n\n")
        ...
        print(f"💾 Resultados guardados en: resultados_casobase_test.txt")
